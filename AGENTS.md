...
## 开发协作规范（git）
- 远程仓库：`https://github.com/zhiyong95/tiger.git`（origin，分支 main）
- **每次改动前必须先 `git pull` 拉取最新代码**，保证在最新版本上进行开发
- 改动完成后推送 `main`，保持本地与远端同步
- 生产构建与推送使用绝对路径 serve dist，避免静态资源回退白屏
