...
export async function generateReport(templateId: string, period: string): Promise<{ title: string; content: string; suggestions: string[] }> {
  await delay(2000)
  return {
    title: `${mockReportTemplates.find((t) => t.id === templateId)?.name || '分析报告'} - ${period}`,
    content: `一、总体情况
${period}，我市就业形势总体稳定，各项指标保持在合理区间。

二、主要指标分析
1. 城镇新增就业：本月新增就业12,580人，同比增长5.2%
2. 失业登记：本月失业登记3,420人，同比下降2.1%
3. 技能培训：本月培训4,230人，同比增长8.5%
...
（详见AI建议部分）`,
    suggestions: [
      '建议加大职业技能培训力度，重点针对制造业和服务业需求',
      '建议完善就业援助政策，加强对困难群体的帮扶',
      '建议优化创业扶持政策，激发市场活力',
    ],
  }
}
